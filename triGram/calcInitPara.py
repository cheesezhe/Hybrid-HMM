...
import string
# 通过训练数据集得到3元模型初始数据
ftrain = open("E:\\PyProj\\Others\\9-1\\rite_HMM_data_train.txt")
N = 16
M = 24
A0 = [([0.0] * N) for i in range(N)]
A = [[([0.0] * N) for i in range(N)] for j in range(N)]
B0 = [([0.0] * M) for i in range(N)]
B = [[([0.0] * M) for i in range(N)] for j in range(N)]
PI0 = [0.0 for i in range(N)]
...
def triGramViterbi(N, M, A, B0, B, PI0, observed):  #observed 是数字序列,且长度最短为3
    ...
    Delta0 = [([0.0] * N) for i in range(2)]
    Delta = [[([0.0] * N) for i in range(N)] for j in range(LEN)]
    path = [([0] * N) for i in range(LEN)]
    #path = [([([([0]*N) for i in range(N)])for j in range(LEN)])]
    #第1天计算，状态的初始概率*隐藏状态到观察状态的条件概率
    for i in range(N):
        Delta0[0][i] = PI0[i] * B0[i][observed[0]]
        path[0][i] = -1
    ...
    for j in range(N):  #跳转状态
        ...
        idx = 0
        for i in range(N):
            if (Delta0[0][i] * A0[i][j] > max):
                max = Delta0[0][i] * A0[i][j]
                idx = i
        xxx = max * B[i][idx][observed[1]]
        Delta[1][idx][j] = xxx
        path[1][j] = idx
    # 第2天以后的计算
    ...
    for t in range(2, LEN):  #时间t
        for k in range(N):  #跳转状态
            ...
            for i in range(N):
                for j in range(N):
                    if (Delta[t - 1][i][j] * A[i][j][k] > max):
                        max = Delta[t - 1][i][j] * A[i][j][k]
                        idx = j
            Delta[t][idx][k] = max * B[idx][k][observed[t]]
            path[t][k] = idx
    #找到最大输出序列
    max = 0.0
    idx = 0
...
ftest = open("E:\\PyProj\\Others\\9-1\\rite_HMM_data_test.txt")
word = ftest.readline()
pos = ftest.readline()
