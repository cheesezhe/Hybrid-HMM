...
import math
#读取loc概率矩阵
floc = open("E:\\PyProj\\Others\\7-3\\locmat.txt")
C = [([0.0] * 11) for i in range(7)]
line = floc.readline().strip()
m = 0
while(line):
    lines = line.split(" \t")
    n = 0
...
def triGramViterbi(N, C, A, B0, B, PI0, observed):  #observed 是数字序列,且长度最短为3
    Clambda = 0.5
    sta = []
    LEN = len(observed)
    if (LEN < 3):
    ...
    for i in range(N):
        Delta0[0][i] = math.log10(PI0[i]) + math.log10(B0[i][observed[0]]) + Clambda*math.log10(C[i][0])
        path[0][i] = -1
    ...
    for j in range(N):  #跳转状态
        ...
        for i in range(N):
            if (Delta0[0][i] +math.log10(A0[i][j])  > max):
                ...
                idx = i
        xxx = max + math.log10(B[idx][j][observed[1]]) + Clambda*math.log10(C[j][1*10/LEN])
        Delta[1][idx][j] = xxx
        path[1][j] = idx
    # 第2天以后的计算
            ...
            for i in range(N):
                for j in range(N):
                    if (Delta[t - 1][i][j] +math.log10(A[i][j][k])  > max):
                        ...
                        idx = j
            Delta[t][idx][k] = max +math.log10(B[idx][k][observed[t]])+Clambda*math.log10(C[k][t*10/LEN])
            path[t][k] = idx
    #找到最大输出序列
    max = -1000.0
    idx = 0
...
while (word):
    ...
    if (len(observed) >= 3):
        ...
        print pos,
        print event,
        sta = triGramViterbi(N, C, A, B0, B, PI0, observed)
        for i in sta:
            print str(i),
        print ' '
    word = ftest.readline()
